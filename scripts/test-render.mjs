...
// ── 116. chartEx detect-only: cx:chartSpace subtype + placeholder ────────
...
//   · emit no <svg> inside the placeholder (detect-only — no renderer yet).
{
    ...
    assert(Array.isArray(sheet.charts) && sheet.charts.length === 2,
        `116a: expected 2 chartEx charts (got ${sheet.charts?.length})`);

    const bySubtype = new Map();
    for (const ch of sheet.charts) {
        assert(ch.kind === 'chartex',
            `116b: SheetChart.kind should be "chartex" for every entry (got ${ch.kind})`);
        assert(ch.model && ch.model.kind === 'chartex',
            `116c: ChartModel.kind should be "chartex" (got ${ch.model?.kind})`);
        if (ch.model) bySubtype.set(ch.model.chartExSubtype, ch);
    }

    const tree = bySubtype.get('treemap');
    const water = bySubtype.get('waterfall');
    assert(tree, '116d: a chart with chartExSubtype="treemap" should be present');
    assert(water, '116e: a chart with chartExSubtype="waterfall" should be present');

    if (tree?.model) {
        assert(tree.model.title === 'Budget by Region',
            `116f: treemap title should round-trip (got ${JSON.stringify(tree.model.title)})`);
        assert(JSON.stringify(tree.model.categories) === JSON.stringify(['Alpha', 'Beta', 'Gamma', 'Delta', 'Epsilon']),
            `116g: treemap categories should round-trip from the first cx:data strDim (got ${JSON.stringify(tree.model.categories)})`);
        assert(tree.model.series.length === 1,
            `116h: treemap should surface one synthesised series (got ${tree.model.series.length})`);
        assert(JSON.stringify(tree.model.series[0]?.values) === JSON.stringify([120, 80, 60, 40, 25]),
            `116i: treemap values should round-trip from the first cx:data numDim (got ${JSON.stringify(tree.model.series[0]?.values)})`);
    }
    if (water?.model) {
        assert(water.model.title === 'Q1 Revenue Bridge',
            `116j: waterfall title should round-trip (got ${JSON.stringify(water.model.title)})`);
        assert(JSON.stringify(water.model.categories) === JSON.stringify(['Opening', 'Sales', 'Returns', 'Closing']),
            `116k: waterfall categories should round-trip (got ${JSON.stringify(water.model.categories)})`);
        assert(JSON.stringify(water.model.series[0]?.values) === JSON.stringify([1000, 450, -120, 1330]),
            `116l: waterfall values should round-trip (got ${JSON.stringify(water.model.series[0]?.values)})`);
    }

    // Renderer: two placeholders (NOT xlsx-chart figures). No SVG inside.
    const figures = container.querySelectorAll('section.xlsx figure.xlsx-chart');
    assert(figures.length === 0,
        `116m: chartEx entries should NOT render as <figure class="xlsx-chart"> (got ${figures.length})`);
    const placeholders = container.querySelectorAll('section.xlsx .xlsx-chart-placeholder');
    assert(placeholders.length === 2,
        `116n: expected 2 .xlsx-chart-placeholder divs (got ${placeholders.length})`);

    const subtypesSeen = new Set();
    for (const ph of placeholders) {
        assert(ph.getAttribute('data-chart-kind') === 'chartex',
            `116o: placeholder data-chart-kind should be "chartex" (got ${ph.getAttribute('data-chart-kind')})`);
        assert(ph.getAttribute('data-chart-plot') === 'chartex',
            `116p: placeholder data-chart-plot should be "chartex" (got ${ph.getAttribute('data-chart-plot')})`);
        const sub = ph.getAttribute('data-chartex-type');
        assert(sub === 'treemap' || sub === 'waterfall',
            `116q: placeholder data-chartex-type should be "treemap" or "waterfall" (got ${sub})`);
        if (sub) subtypesSeen.add(sub);
        assert(ph.querySelector('svg') === null,
            `116r: detect-only wave — no <svg> should sit inside a chartEx placeholder (got one)`);
    }
    assert(subtypesSeen.has('treemap') && subtypesSeen.has('waterfall'),
        `116s: both treemap + waterfall placeholders should surface (got ${[...subtypesSeen].join(',')})`);
}
