...
// ── 111. formulas-v2: end-to-end evaluateFormulas for the wave-12 surface ─
...
// this same fixture.
{
    ...
    assert(e(0)?.value === '42', `111a: E1 IFERROR(1/0, 42) → 42 (got ${e(0)?.value})`);
    assert(e(0)?.kind === 'number', `111a2: E1 kind should be number`);

    assert(e(1)?.value === 'two', `111b: E2 SWITCH(2, 1,"one", 2,"two", "other") → "two" (got ${e(1)?.value})`);

    assert(e(2)?.value === 'second', `111c: E3 VLOOKUP("beta", B1:C3, 2) → "second" (got ${e(2)?.value})`);

    assert(e(3)?.value === '1.23', `111d: E4 ROUND(1.2345, 2) → 1.23 (got ${e(3)?.value})`);

    // SUMIF over D1:D5 with criteria "foo", summing A1:A5:
    //   foo rows → A values 10, 30, 15 → 55
    assert(e(4)?.value === '55', `111e: E5 SUMIF(D1:D5, "foo", A1:A5) → 55 (got ${e(4)?.value})`);

    assert(e(7)?.value === 'hello world', `111f: E8 TRIM("  hello   world  ") → "hello world" (got ${JSON.stringify(e(7)?.value)})`);
    assert(e(8)?.value === 'MIXED CASE', `111g: E9 UPPER("mixed case") → "MIXED CASE" (got ${e(8)?.value})`);
    ...
    assert(tdOf(0, 4).textContent === '42', `111h: E1 renders as 42 (got ${tdOf(0, 4).textContent})`);
    assert(tdOf(0, 4).classList.contains('xlsx-numeric'), '111i: E1 should carry xlsx-numeric');
    assert(tdOf(1, 4).textContent === 'two', `111j: E2 renders as "two" (got ${tdOf(1, 4).textContent})`);
    assert(!tdOf(1, 4).classList.contains('xlsx-numeric'), '111k: E2 string result should not be numeric-aligned');
}

// ── 112. formulas-v2: error taxonomy — each new code surfaces on its cell ─
...
// Excel sentinel. The renderer paints `xlsx-error` on each.
{
    ...
    for (const [rowStr, code] of Object.entries(expected)) {
        ...
        const cell = e(r);
        assert(cell?.kind === 'error', `112a.${code}: E${r + 1} kind should be error (got ${cell?.kind})`);
        assert(cell?.value === code, `112b.${code}: E${r + 1} value should be ${code} (got ${cell?.value})`);
    }
    ...
    for (const code of Object.values(expected)) {
        assert(seen.has(code), `112c: expected error code ${code} to appear at least once in the sheet`);
    }
    ...
    for (const [rowStr, code] of Object.entries(expected)) {
        ...
        assert(td.textContent === code, `112d.${code}: E${r + 1} renders as ${code} (got ${td.textContent})`);
        assert(td.classList.contains('xlsx-error'), `112e.${code}: E${r + 1} should carry xlsx-error`);
    }
}

// ── 113. formulas-v2: number-format routing — 0.1+0.2 renders as 0.3 ──────
...
// kind:'number' result flows into formatNumber for explicit numFmts.
{
    ...
    assert(e6?.value === '0.3', `113a: E6 =0.1+0.2 cell.value should be "0.3" (got ${JSON.stringify(e6?.value)})`);
    assert(e6?.kind === 'number', `113b: E6 cell.kind should be number (got ${e6?.kind})`);

    // E7 — explicit "0.00" format. cell.value is the raw "0.3"; the
    // formatNumber pipeline adds the trailing zero.
    assert(e7?.value === '0.3', `113c: E7 =0.1+0.2 cell.value should still be "0.3" raw (got ${JSON.stringify(e7?.value)})`);
    assert(e7?.kind === 'number', `113d: E7 cell.kind should be number (got ${e7?.kind})`);

    const tbodyRows = container.querySelectorAll('section.xlsx tbody tr');
    const tdOf = (r, c) => tbodyRows[r].querySelectorAll('td')[c];
    // Row 5 is E6, row 6 is E7.
    assert(tdOf(5, 4).textContent === '0.3',
        `113e: E6 General should render as "0.3" (got ${JSON.stringify(tdOf(5, 4).textContent)})`);
    assert(tdOf(6, 4).textContent === '0.30',
        `113f: E7 with "0.00" format should render as "0.30" (got ${JSON.stringify(tdOf(6, 4).textContent)})`);
}
