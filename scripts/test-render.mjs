...
// ── 115. slicer re-materialisation: opt-in handler hides non-matching rows ─
...
// Default-off leaves every row visible even when the same click fires.
{
    ...
    const section = container.querySelector('section.xlsx');
    assert(section !== null, '115a: section.xlsx should be present');
    ...
    assert(totalRows === 5, `115b: fixture should render 5 tbody rows (1 header + 4 data; got ${totalRows})`);
    assert(visibleRows().length === totalRows,
        `115c: all rows should be visible before any click (got ${visibleRows().length}/${totalRows})`);
    ...
    assert(slicerAside !== null, '115d: interactive slicer aside should exist');
    ...
    assert(!!northChip, '115e: North chip should be present');
    assert(northChip?.getAttribute('aria-pressed') === 'true',
        `115f: North chip should start pressed (got ${JSON.stringify(northChip?.getAttribute('aria-pressed'))})`);

    northChip?.dispatchEvent(new window.MouseEvent('click', { bubbles: true }));
    assert(visibleRows().length === totalRows - 1,
        `115g: after unpressing North, exactly one row should be hidden (visible=${visibleRows().length}, expected ${totalRows - 1})`);
    ...
    assert(hidden.length === 1, `115h: exactly one row hidden (got ${hidden.length})`);
    const hiddenRegion = hidden[0]?.querySelectorAll('td')[0]?.textContent;
    assert(hiddenRegion === 'North',
        `115i: hidden row should be the North row (got ${JSON.stringify(hiddenRegion)})`);

    // Re-click North → it becomes pressed again; all rows come back.
    northChip?.dispatchEvent(new window.MouseEvent('click', { bubbles: true }));
    assert(visibleRows().length === totalRows,
        `115j: after re-pressing North, all rows should re-appear (visible=${visibleRows().length}, expected ${totalRows})`);
}

// ── 116. slicer re-materialisation: default-off leaves rows visible ─────
...
// full control of the pivot DOM.
{
    ...
    assert(!!chip, '116a: North chip should exist under interactiveSlicers alone');

    chip?.dispatchEvent(new window.MouseEvent('click', { bubbles: true }));
    const hidden = [...(section?.querySelectorAll('tbody > tr') ?? [])]
        .filter((tr) => tr.style.display === 'none');
    assert(hidden.length === 0,
        `116b: without slicerRematerializePivots, no rows should be hidden after the click (got ${hidden.length} hidden)`);
}

// ── 117. slicer re-materialisation: cross-sheet source → skip + warn ────
...
// still fires on click, so consumers can still drive their own filter UI.
{
    ...
    assert(warns.length === 0,
        `117a: no console.warn expected on a resolvable source; got ${JSON.stringify(warns)}`);
}
