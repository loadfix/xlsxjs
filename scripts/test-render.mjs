...
// ── 115. smartart-matrix-pyramid: matrix + pyramid native layouts ─────────
...
// off the polygons' points attribute.
{
    ...
    const sheet = wb.parsed.sheets[0];
    assert(Array.isArray(sheet.smartArt), '115a: Sheet.smartArt should be an array');
    assert(sheet.smartArt.length === 2,
        `115b: expected 2 SmartArt entries (got ${sheet.smartArt.length})`);

    const layouts = sheet.smartArt.map((a) => a.model?.layout ?? '');
    assert(layouts.some((l) => /matrix/i.test(l)),
        `115c: one layout should match /matrix/i (got ${JSON.stringify(layouts)})`);
    assert(layouts.some((l) => /pyramid/i.test(l)),
        `115d: one layout should match /pyramid/i (got ${JSON.stringify(layouts)})`);
    ...
    assert(matrix, '115e: matrix entry should be present');
    assert(pyramid, '115f: pyramid entry should be present');
    ...
    if (matrix && matrix.model) {
        ...
        assert(JSON.stringify(names) === JSON.stringify(['Revenue', 'Growth', 'Quality', 'Speed']),
            `115g: matrix roots should be [Revenue, Growth, Quality, Speed] (got ${JSON.stringify(names)})`);
    }
    if (pyramid && pyramid.model) {
        ...
        assert(JSON.stringify(names) === JSON.stringify(['Vision', 'Strategy', 'Execution', 'Review']),
            `115h: pyramid roots should be [Vision, Strategy, Execution, Review] (got ${JSON.stringify(names)})`);
    }
    ...
    const asides = container.querySelectorAll('section.xlsx aside.xlsx-smartart');
    assert(asides.length === 2,
        `115i: expected 2 <aside class="xlsx-smartart"> (got ${asides.length})`);
    ...
    assert(matrixAside, '115j: matrix aside should be present');
    assert(pyramidAside, '115k: pyramid aside should be present');

    if (matrixAside) {
        const svg = matrixAside.querySelector(':scope > svg');
        assert(!!svg, '115l: matrix aside should contain an <svg>');
        if (svg) {
            const rects = svg.querySelectorAll('rect');
            assert(rects.length >= 4,
                `115m: matrix SVG should carry >=4 <rect> node boxes (got ${rects.length})`);
            const lines = svg.querySelectorAll('line');
            assert(lines.length >= 2,
                `115n: matrix SVG should carry >=2 <line>s for the central cross (got ${lines.length})`);
            const labels = [...svg.querySelectorAll('text')].map((t) => t.textContent);
            assert(labels.includes('Revenue'),
                `115o: matrix should contain a "Revenue" label (got ${JSON.stringify(labels)})`);
            assert(labels.includes('Quality'),
                `115p: matrix should contain a "Quality" label (got ${JSON.stringify(labels)})`);
        }
    }

    if (pyramidAside) {
        const svg = pyramidAside.querySelector(':scope > svg');
        assert(!!svg, '115q: pyramid aside should contain an <svg>');
        if (svg) {
            const polygons = [...svg.querySelectorAll('polygon')];
            assert(polygons.length >= 4,
                `115r: pyramid SVG should carry >=4 <polygon> bands (got ${polygons.length})`);
            const labels = [...svg.querySelectorAll('text')].map((t) => t.textContent);
            assert(labels.includes('Vision'),
                `115s: pyramid should contain a "Vision" label at the top (got ${JSON.stringify(labels)})`);
            assert(labels.includes('Review'),
                `115t: pyramid should contain a "Review" label at the bottom (got ${JSON.stringify(labels)})`);
            ...
            if (polygons.length >= 2) {
                ...
                assert(topSpan < bottomSpan,
                    `115u: top polygon x-span (${topSpan}) should be narrower than bottom polygon x-span (${bottomSpan})`);
            }
        }
    }
}
