...
// Column A holds a mix of negative + positive numbers so the negative fill
// has something to match; a containsText rule on B2:B4 fires a dxf with
// <strike/> + numFmtCode "0.00" so those cells get crossed out and the
// value re-formatted from the dxf's code.
//
...
//   A4 =  5   B4 = "quiet"
//
// Numeric cells in col B pick up a styleIndex that carries a number
// so the dxf's numFmt has something to re-format — but the containsText
// rule actually fires on the string cells (where dxf numFmt is a no-op).
// To exercise the numFmt path end-to-end we put numeric cells in col C
// that match a separate rule (cellIs >= 0) using the same dxf.
{
...
<Relationships xmlns="http://schemas.openxmlformats.org/package/2006/relationships">
  ...
  <Relationship Id="rId3" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/styles" Target="styles.xml"/>
    ...
    // dxfs[0] = strike + numFmtCode "0.00". The containsText "flag" rule
    // points at it; any cell whose text contains "flag" gets the dxf applied,
    // and any numeric cell whose cellIs rule fires gets the numFmt reapplied.
    zip.file('xl/styles.xml', `<?xml version="1.0" encoding="UTF-8" standalone="yes"?>
<styleSheet xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main">
  <fonts count="1"><font><sz val="11"/><name val="Calibri"/></font></fonts>
  <fills count="1"><fill><patternFill patternType="none"/></fill></fills>
  ...
  <dxfs count="1">
    ...
    // Sheet XML: cfRule type="dataBar" on A1:A4 with an <extLst>/<ext>/<x14:id>
    // pointing to the GUID, and a sheet-level <extLst>/<x14:cfRule id="{GUID}"/>
    // block carrying the post-2010 attributes (axisPosition="middle",
    // border="1", negativeFillColor, borderColor, axisColor).
    // Also a containsText rule on B1:B4 and a cellIs >= 0 rule on C1:C4,
    // both with dxfId=0 (strike + numFmt "0.00").
