...
import type { Workbook, Sheet, SheetView, Cell, MergedRange, RichTextRun, FrozenPanes, SheetComment, SheetShape, SheetImage, SheetEmbedding, RowDimension, ThreadedCommentEntry, Hyperlink, PhoneticRun } from './workbook-parser';
...
function renderSheet(sheet: Sheet, styles: Styles | null, theme: Theme | null, date1904: boolean, options: Options): HTMLElement {
    ...
    // Anchored embeddings join the image overlay when they carry col/row
    // coordinates — `<xdr:graphicFrame>`-style OLE refs typically do. Unanchored
    // entries fall through to the section-level render step after the table so
    // they still reach the DOM (useful for legacy producers that omit anchors).
    ...
    if (sheet.images.length > 0 || anchoredEmbeddings.length > 0) {
