...
export class WorkbookParser {
    ...
    parse(parts: Record<string, string>, media: Record<string, string> = {}): Workbook {
        ...
        for (let i = 0; i < sheetMeta.length; i++) {
            ...
            sheets.push(parseSheet(name, xml, sharedStrings, tables, images, comments));
        }
...
function parseSheet(name: string, xml: string, sharedStrings: SharedString[], tables: TableDef[] = [], images: SheetImage[] = [], comments: SheetComment[] = []): Sheet {
    const doc = parseXml(xml);
